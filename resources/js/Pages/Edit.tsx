...
export default function Edit({ report, services }: any) {
    const { data, setData, put, errors } = useForm({
        name: report.name,
        email: report.email,
        no_handphone: report.no_handphone,
        service_id: report.service_id,
        service_type_id: report.service_type_id,
    ...
    return (
        <main className='container h-screen mt-10'>
            <Head title='Edit' />
            <h1 className="text-base-content text-4xl mb-5">Edit Data Laporan</h1>
            ...
            <form onSubmit={onSubmit}>
                ...
                <div className="relative w-full mb-6">
                    <select className="select select-floating max-w-sm" aria-label="Select floating label" id="service_id" onChange={handleChange}>
                        ...
                        {services.map((service: any) => (
                            <option key={service.id} value={service.id} selected={service.id === data.service_id}>{service.name}</option>
                        ))}
                    </select>
                ...
                <div className="relative w-full mb-6">
                    <select className="select select-floating max-w-sm" aria-label="Select floating label" id="service_type_id" onChange={handleChange}>
                        ...
                        {services[data.service_id - 1].service_types.map((service_type: any) => (
                            <option key={service_type.id} value={service_type.id} selected={service_type.id === data.service_type_id}>{service_type.name}</option>
                        ))}
                    </select>
                ...
                <div className="relative w-full mb-4">
                    <select className="select select-floating max-w-sm" aria-label="Select floating label" id="status" onChange={handleChange}>
                        ...
                        <option key={'penggugat'} value={'penggugat'} selected={'penggugat' === data.status}>Penggugat</option>
                        <option key={'tergugat'} value={'tergugat'} selected={'tergugat' === data.status}>Tergugat</option>
                    </select>
                ...
                <button className="btn btn-soft btn-primary" type='submit'>Edit</button>
            </form>
        </main>
    )
}
